...
In order to configure Sonarcloud analysis [GitHub Action workflow](https://github.com/snorben/eeg_preprocessing_umcu/blob/main/.github/workflows/sonarcloud.yml) you must follow the steps below:
...
1. go to [new code definition administration page](https://sonarcloud.io/project/new_code?id=snorben_eeg_preprocessing_umcu) and select `Number of days` option
1. To be able to run the analysis:
   1. a token must be created at [Sonarcloud account](https://sonarcloud.io/account/security/)
   1. the created token must be added as `SONAR_TOKEN` to [secrets on GitHub](https://github.com/snorben/eeg_preprocessing_umcu/settings/secrets/actions)
